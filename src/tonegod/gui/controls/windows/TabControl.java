...
/**
 * Implements a tab control in which combinations of a tab button and a panel can be added.
 * <p>
 * At least one tab panel must be added before trying to display the control [use {@link #addTab(String) } functions].
...
public abstract class TabControl extends Element {
    ...
    private Vector2f pos = new Vector2f(), dim = new Vector2f();
...
//			tabSlider.setTrayPadding(-(tabResizeBorders.z/5*2));
    ...
    /**
     * Get the tab panel element by its index.
     *
    ...
    public Element getTab(int index) throws ArrayIndexOutOfBoundsException {
        if (tabPanels.containsKey(index)) {
            return (Element) tabPanels.get(index);
        } else {
